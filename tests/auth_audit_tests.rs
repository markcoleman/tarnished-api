...
use tarnished_api::{
    create_base_app, AuthAuditEvent, AuthEventType, AuthEventOutcome,
...
async fn test_login_success_audit_logging() {
    ...
    // Parse JSON response
    let json: LoginResponse = serde_json::from_str(body_str)
        .expect("Failed to parse response as JSON");
    
    // Check that login was successful
    assert!(json.success, "Login should be successful");
    assert!(json.token.is_some(), "Token should be present");
    assert!(json.token.unwrap().starts_with("token_"), "Token should have expected format");
    assert_eq!(json.message, "Login successful", "Message should indicate success");
}
...
async fn test_token_validation_success_audit_logging() {
    ...
    // Verify response status is 200 OK
    assert_eq!(resp.status(), StatusCode::OK, "Token validation should succeed");
    ...
    // Parse JSON response
    let json: TokenValidationResponse = serde_json::from_str(body_str)
        .expect("Failed to parse response as JSON");
    ...
    assert_eq!(json.message, "Token is valid", "Message should indicate success");
}
...
async fn test_token_validation_failure_audit_logging() {
    ...
    assert_eq!(resp.status(), StatusCode::OK, "Endpoint should respond normally");
    ...
    assert_eq!(json.message, "Invalid token", "Message should indicate failure");
}
...
async fn test_audit_event_creation() {
    ...
    assert!(json.contains("login_success"), "JSON should contain event type");
    assert!(json.contains("success"), "JSON should contain outcome");
    assert!(json.contains("192.168.1.100"), "JSON should contain IP address");
    assert!(json.contains("test_user"), "JSON should contain user ID");
    
    // Verify event can be deserialized from JSON
    let deserialized: AuthAuditEvent = serde_json::from_str(&json)
...
async fn test_suspicious_activity_detection() {
    ...
    assert!(!tracker.is_suspicious(test_ip), "IP should not be suspicious initially");
    
    // Record multiple failures
    for i in 0..4 {
        let is_suspicious = tracker.record_failure(test_ip);
        assert!(!is_suspicious, "Should not be suspicious after {} failures", i + 1);
    }
    ...
    assert!(tracker.is_suspicious(test_ip), "IP should be flagged as suspicious");
    
    // Different IP should not be affected
    assert!(!tracker.is_suspicious("192.168.1.101"), "Different IP should not be suspicious");
}