...
async fn test_health_endpoint_integration() {
    ...
    assert_eq!(
        json, expected_json,
        "Response JSON should match expected structure"
    );
...
async fn test_version_endpoint_integration() {
    ...
    assert!(
        build_time.unwrap().is_string(),
        "Build time should be a string"
    );
...
async fn test_rate_limiting_integration() {
    ...
    assert_eq!(
        health_resp3.status(),
        StatusCode::OK,
        "Health endpoint should continue to work"
    );
...
async fn test_rate_limiter_unit() {
    ...
    assert!(
        limiter.check_rate_limit("different_ip"),
...
async fn test_hmac_signature_validation() {
    ...
    let old_signature = hmac_utils::generate_signature(secret, payload, old_timestamp)
        .expect("Should generate signature");
    let result =
        hmac_utils::validate_signature(secret, payload, old_timestamp, &old_signature, 300)
            .expect("Validation should not error");
    ...
    let future_signature = hmac_utils::generate_signature(secret, payload, future_timestamp)
        .expect("Should generate signature");
    let result =
        hmac_utils::validate_signature(secret, payload, future_timestamp, &future_signature, 300)
            .expect("Validation should not error");
    assert!(
        result,
        "Future timestamp within tolerance should pass validation"
    );
}
...
async fn test_hmac_middleware_not_required() {
    ...
    assert_eq!(
        resp.status(),
        StatusCode::OK,
        "Request should succeed when HMAC not required"
    );
...
async fn test_hmac_middleware_required_missing_headers() {
    ...
    assert_eq!(
        ...
        "Request should fail when HMAC required but missing headers"
    );
...
async fn test_hmac_middleware_valid_signature() {
    ...
    assert_eq!(
        resp.status(),
        StatusCode::OK,
        "Request should succeed with valid signature"
    );
...
async fn test_response_signature_functionality() {
    ...
    assert!(
        ...
        "Response should have X-Signature header"
    );
    assert!(
        timestamp_header.is_some(),
        "Response should have X-Timestamp header"
    );
    ...
    let is_valid =
        hmac_utils::validate_signature(&config.secret, response_body, timestamp, signature, 300)
...
async fn test_security_headers_health_endpoint() {
    ...
    assert!(
        csp.is_some(),
        "Content-Security-Policy header should be present when enabled"
    );
    assert_eq!(
        csp.unwrap().to_str().unwrap(),
...
async fn test_csp_disabled() {
    ...
    assert!(
        csp.is_none(),
        "Content-Security-Policy header should not be present when disabled"
    );
...
async fn test_request_id_header_added() {
    ...
    assert_eq!(
        request_id.len(),
        36,
        "Request ID should be 36 characters long (UUID format)"
    );
    assert_eq!(
...
async fn test_request_id_header_preserved() {
    ...
    assert_eq!(
        returned_request_id, existing_request_id,
        "Response should preserve the original Request ID"
    );
}
...
async fn test_request_id_on_version_endpoint() {
    ...
    assert!(
        request_id_header.is_some(),
        "Version endpoint response should contain X-Request-ID header"
    );
...
async fn test_metrics_endpoint_integration() {
    ...
    assert_eq!(
        metrics_resp.status(),
        StatusCode::OK,
        "Expected 200 OK status for metrics endpoint"
    );
    ...
    assert!(
        body_str.contains("app_uptime_seconds"),
        "Should contain app_uptime_seconds metric"
    );
    assert!(
        body_str.contains("app_info"),
        "Should contain app_info metric"
    );

    // Check for application version info in metrics
    assert!(
        body_str.contains("version=\"0.1.0\""),
    ...
    assert!(
        ...
        "Should contain metric help text"
    );
    assert!(
        body_str.contains("# TYPE"),
        "Should contain metric type information"
    );

    // Verify that we have some actual metric values
    assert!(
        body_str.matches(char::is_numeric).count() > 0,
        "Should contain numeric metric values"
    );
}
...
async fn test_metrics_endpoint_disabled() {
    ...
    assert!(
        ...
        "Response should indicate metrics are disabled"
    );
}
